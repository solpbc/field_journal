# attribution

Detailed license and attribution information for each media source included in the field journal.

## AMI Meeting Corpus

- **Provider:** University of Edinburgh, Idiap Research Institute, University of Twente
- **URL:** https://groups.inf.ed.ac.uk/ami/corpus/
- **License:** CC-BY 4.0 International
- **Attribution required:** Yes
- **Share-alike:** No
- **Usage:** Multi-party meeting recordings used as audio segments

## Paradigm Shift AI Computer Use Dataset

- **Provider:** Paradigm Shift AI
- **URL:** https://huggingface.co/datasets/anaisleila/computer-use-data-psai
- **License:** MIT
- **Attribution required:** Yes (license notice)
- **Share-alike:** No
- **Usage:** Desktop screen capture recordings used as screen segments

## Library of Congress American English Dialect Recordings

- **Provider:** Library of Congress, American Folklife Center
- **URL:** https://citizen-dj.labs.loc.gov/loc-american-english-dialect-recordings/use/
- **License:** Free to use and reuse, no restrictions
- **Attribution required:** Recommended but not required
- **Share-alike:** No
- **Usage:** Natural conversation and dialect interview recordings used as audio segments

## NASA Press Conferences and Technical Briefings

- **Provider:** NASA
- **URL:** https://images.nasa.gov/
- **License:** Public domain (US government work)
- **Attribution required:** No
- **Share-alike:** No
- **Usage:** Multi-speaker briefing recordings used as audio segments

## Hacker Public Radio

- **Provider:** Hacker Public Radio community
- **URL:** https://hackerpublicradio.org/
- **License:** CC-BY-SA 4.0
- **Attribution required:** Yes
- **Share-alike:** Yes
- **Usage:** Solo tech commentary episodes used as audio segments

## CHiME-6 Dinner Parties

- **Provider:** University of Edinburgh CHiME Challenge
- **URL:** https://www.chimechallenge.org/challenges/chime6/
- **Mirror:** https://huggingface.co/datasets/argmaxinc/chime-6
- **License:** CC-BY-SA 4.0
- **Attribution required:** Yes
- **Share-alike:** Yes
- **Usage:** Far-field dinner-party recordings used as audio segments and reference transcripts

## ICSI Meeting Corpus

- **Provider:** International Computer Science Institute / University of Edinburgh
- **URL:** https://groups.inf.ed.ac.uk/ami/icsi/
- **Mirror:** https://huggingface.co/datasets/argmaxinc/icsi-meetings
- **License:** CC-BY 4.0
- **Attribution required:** Yes
- **Share-alike:** No
- **Usage:** Recurring lab meeting recordings used as audio segments and reference transcripts

## VOiCES Corpus

- **Provider:** SRI International / Lab41 (IQT Labs)
- **URL:** https://iqtlabs.github.io/voices/
- **Mirror:** https://github.com/Lab41/VOiCES-subset
- **License:** CC-BY 4.0 International
- **Attribution required:** Yes
- **Share-alike:** No
- **Usage:** Far-field replayed LibriSpeech utterances at multiple microphone distances used as audio segments for cross-device drift evaluation
